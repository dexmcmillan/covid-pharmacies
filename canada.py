...
# Read in data.
path = '' # use your path
all_files = glob.glob(path + "./data/geocoded/*.csv")
...
df_pharmacies = pd.read_csv('./data/data-ontario-pharmacies.csv')
df_pharmacies["covid_site"] = "No"

df.append(df_pharmacies)

# df_pharmacies.rename(columns={"POSTALCODE": "postal", "EN_NAME": "name", "COMMUNITY": "city"}, inplace=True)
# combined = df.append(df_pharmacies)

data = df[["name", 'city', 'province', 'covid_site', "street", "date", "X", "Y", "postal", "fsa", "location_code", "type"]]
...
toprint.to_csv('./data/clean/output-canada.csv')